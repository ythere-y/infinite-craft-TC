...
def test_wall_assets_share_current_cache_version():
    ...
    assert asset_urls == [
        "/icon-system.css?v=20260731b",
        "/wall/wall.css?v=20260731b",
        "/icon-system.js?v=20260731b",
        "/wall/wall.js?v=20260731b",
    ]
    assert dependency_urls == [
        "./polling.js?v=20260731b",
        "./first-honor.js?v=20260731b",
        "./recipe-comments.js?v=20260731b",
    ]
...
def test_wall_renders_one_ordered_leaderboard_and_first_honor(tmp_path):
    actual = _run_wall(tmp_path, setup=LEADERBOARD_SETUP, probe="""
      var lists = Array.from(document.querySelectorAll(".lb-ranking-list"));
      var rows = Array.from(document.querySelectorAll(".lb-ranking-list > .lb-row"));
      return {
        title: document.querySelector(".side-title").textContent.trim(),
        listCount: lists.length,
        listTag: lists[0] && lists[0].tagName,
        ranks: rows.map(function (row) { return row.dataset.rank; }),
        rankLabels: rows.map(function (row) {
          return row.querySelector(".lb-rank").textContent.trim();
        }),
        firsts: rows.map(function (row) {
          return row.querySelector(".lb-firsts").textContent.trim();
        }),
        firstHonor: rows[0].querySelector(".lb-honor").getAttribute("aria-label"),
        firstHonorItems: Array.from(
          rows[0].querySelectorAll(".lb-honor-item")
        ...
        currentHighlighted: rows[3].classList.contains("me")
      };
    """)
    assert actual == {
        "title": "🏆 排行榜 · Top 20",
        ...
        "ranks": ["1", "2", "3", "4"],
        "rankLabels": ["🥇", "🥈", "🥉", "4"],
        "firsts": ["76 个首发", "64 个首发", "16 个首发", "4 个首发"],
...
""", probe="""
  var honor = document.querySelector(".lb-row .lb-honor");
  return {
    aggregated: honor.classList.contains("aggregated"),
    items: Array.from(honor.querySelectorAll(".lb-honor-item"))
      .map(function (item) { return item.textContent; }),
    count: document.querySelector(".lb-row .lb-firsts").textContent.trim()
  };
""")
    assert actual == {
        "aggregated": True,
...
def test_wall_keeps_rank_color_and_current_user_accent_in_top_three(tmp_path):
    actual = _run_wall(tmp_path, setup="""
localStorage.setItem("ic_nick", "当前鹅");
window.__leaderboardPayload = {
  total_players: 3,
  me: { rank: 2, firsts: 64 },
  top: [
    { rank: 1, discoverer: "第一名鹅", firsts: 76 },
    { rank: 2, discoverer: "当前鹅", firsts: 64 },
    { rank: 3, discoverer: "第三名鹅", firsts: 16 }
  ]
};
""", probe="""
  ...
  var current = rows[1];
  return {
    currentIsSecond: current.dataset.rank,
    currentHighlighted: current.classList.contains("me"),
    medal: current.querySelector(".lb-rank").textContent.trim(),
    background: getComputedStyle(current).backgroundColor,
    firstBackground: getComputedStyle(rows[0]).backgroundColor,
    borderLeftWidth: getComputedStyle(current).borderLeftWidth
  };
""")
    assert actual == {
        "currentIsSecond": "2",
        ...
        "firstBackground": "rgb(255, 248, 220)",
        "borderLeftWidth": "3px",
    }


def test_wall_leaderboard_desktop_rows_share_geometry_and_rank_colors(tmp_path):
    actual = _run_wall(
        tmp_path,
        ...
        probe="""
          ...
          var rects = rows.map(function (row) { return row.getBoundingClientRect(); });
          var colors = rows.slice(0, 3).map(function (row) {
            return getComputedStyle(row).backgroundColor;
          });
          return {
            verticalOrder: rects.every(function (rect, index) {
              return index === 0 || rect.top > rects[index - 1].top;
            }),
            equalWidths: rects.every(function (rect) {
              return Math.abs(rect.width - rects[0].width) < 0.5;
            }),
            alignedLeft: rects.every(function (rect) {
              return Math.abs(rect.left - rects[0].left) < 0.5;
            }),
            distinctTopColors: new Set(colors).size,
            fiveColumns: getComputedStyle(honor).gridTemplateColumns.trim().split(/\\s+/).length,
            sideOverflow: side.scrollWidth > side.clientWidth,
            rowsInsideSide: rects.every(function (rect) {
              return rect.left >= sideRect.left && rect.right <= sideRect.right + 1;
            })
          };
        """,
    )
    ...
    actual = _run_wall(
        ...
        probe="""
          ...
          return {
            ...
            visibleRows: rects.every(function (rect) {
                return rect.width > 0 &&
